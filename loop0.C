...
int main(int argc, char* argv[]) {
  ...
  for (unsigned int mcs = 0; mcs < therm + sweeps; ++mcs) {
    ...
    fragments.resize(0); fragments.resize(lattice.num_sites());
    for (int s = 0; s < lattice.num_sites(); ++s) current[s] = s;
    ...
    for (int s = 0; s < lattice.num_sites(); ++s) unify(fragments, s, current[s]);
