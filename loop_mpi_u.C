...
typedef looper::parallel_cluster_unifier<estimate_un_t, collector_un_t> unifier_t;
...
int main(int argc, char* argv[]) {
  ...
  timer.registrate( 2, 3, "_Monte_Carlo_steps");
  timer.registrate( 3, 1, "__initialize_operators");
  timer.registrate( 4, 1, "__fill_times");
  timer.registrate( 5, 1, "__initialize_cluster_infomation");
  timer.registrate( 6, 1, "__for_insert_or_remove_operators");
  timer.registrate( 7, 1, "__for_fragments_current_unify");
  timer.registrate( 8, 1, "__for_fragments_unify");
  timer.registrate( 9, 1, "_unifier_constructor");
  timer.registrate(10, 1, "__for_assign_cluster_id");
  timer.registrate(11, 1, "__for_accumulate_estimates");
  timer.registrate(12, 1, "__for_accumulate_coll");
  timer.registrate(13, 1, "__cluster_flip_or_not");
  timer.registrate(14, 1, "__global_unification_of_open_clusters");
  timer.registrate(15, 1, "__flip_operator_and_spins");
  ...
  std::vector<std::vector<estimate_un_t> > estimates_g(num_threads);
  ...
  for (unsigned int mcs = 0; mcs < therm + sweeps; ++mcs) {
    ...
    #endif
    {
      ...
      if (process_id == 0) {
        ...
        #endif
        for (int s = 0; s < lattice.num_sites(); ++s) {
          coll.umag_0 += 1 - 2 * spins_c[s];
          coll.smag_0 += lattice.phase(s) * (1 - 2 * spins_c[s]);
          coll.smag_a -= tau0 * lattice.phase(s) * (1 - 2 * spins_c[s]);
        }
      } else {
        #ifdef LOOPER_OPENMP
        #pragma omp for schedule(static)
    ...
    #endif
    {
      ...
      for (std::vector<local_operator_t>::iterator opi = operators_p.begin();
        ...
        if (oi->type == offdiagonal) {
          coll.smag_a += 2 * t * lattice.phase(s0) * (1 - 2 * spins_c[s0]);
          coll.smag_a += 2 * t * lattice.phase(s1) * (1 - 2 * spins_c[s1]);
          spins_c[s0] ^= 1;
          spins_c[s1] ^= 1;
        }
      ...
      #endif
      for (int s = 0; s < lattice.num_sites(); ++s) {
        coll.smag_a += tau1 * lattice.phase(s) * (1 - 2 * spins_c[s]);
      }
    ...
    timer.start(12);
    #ifdef LOOPER_OPENMP
    std::vector<estimate_un_t>& estimates = estimates_g[0];
    #endif
