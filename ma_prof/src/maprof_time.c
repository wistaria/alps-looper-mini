...
void maprof_print(int id, const char *name)
{
  ...
  if (time > 0.0) {
    ...
    printf("%s Time:                 unknown\n", name);
  }
  if (time > 0.0 && flops > 0.0) {
    printf("%s FLOPS:                %g (GFLOPS)\n", name, flops);
  } else {
    printf("%s FLOPS:                unknown\n", name);
  }
  if (time > 0.0 && tput > 0.0) {
    printf("%s Throughput:           %g (GB/S)\n", name, tput);
  } else {
    printf("%s Throughput:           unknown\n", name);
  }
  if (time > 0.0 && etput > 0.0) {
    printf("%s Effective Throughput: %g (GB/S)\n", name, etput);
  } else {
    printf("%s Effective Throughput: unknown\n", name);
  }
}
